...
var TOTAL_POSTS = 0;
var filesInDirectory = { video: [], photo: [] };
 ...
 * @returns
 */
var _getAllFilesFromFolder = function (dir, site) {
    try {
        filesInDirectory = require("".concat(dir, "/").concat(site, ".json"));
    }
    catch (e) {
...
var saveToFile = function (fileName, data, dir) {
    fs.writeFile("".concat(dir, "/").concat(fileName, ".json"), JSON.stringify(data), function (error) {
        if (error)
 ...
 * @returns boolean
 */
var isFileAlreadyExist = function (file, type) {
    var isExist = false;
    if (type === "video") {
        filesInDirectory.video.map(function (f) {
            if (file.includes(f))
                isExist = true;
        });
    }
    else if (type === "photo") {
        filesInDirectory.photo.map(function (f) {
            if (file.includes(f))
                isExist = true;
        });
    }
...
var tumblrDownloader = function (site, type, size, start, directory) {
    ...
    getMediaLinks(site, type, size, start).then(function (res) {
        if (res) {
            res.map(function (_medialink) { return __awaiter(void 0, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (!!isFileAlreadyExist(_medialink, type)) return [3 /*break*/, 2];
                            return [4 /*yield*/, axios_1.default
                                    .get(_medialink)
                                    .then(function (_res) { return __awaiter(void 0, void 0, void 0, function () {
                                    return __generator(this, function (_a) {
                                        switch (_a.label) {
                                            case 0: return [4 /*yield*/, fileDownloader(_medialink, directory)];
                                            case 1:
                                                _a.sent();
                                                console.log("done");
                                                return [2 /*return*/];
